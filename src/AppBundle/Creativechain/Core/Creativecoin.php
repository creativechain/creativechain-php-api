...
class Creativecoin{
    ...
    public function getTransactionData($txid) {
        ...
        foreach($decoraw['result']['vout'] as $key=>$value){
            if($value['scriptPubKey']['hex']){
                $str = '';
                for($i=0;$i<strlen($value['scriptPubKey']['hex']);$i+=2) $str .= chr(hexdec(substr($value['scriptPubKey']['hex'],$i,2)));
                $opdataP = $str;
                $opdataP = explode("-CREA-",$opdataP);
                $opdata .= $opdataP[1];
            }
        ...
        $str = '';
        for($i=0;$i<strlen($opdata);$i+=2) $str .= chr(hexdec(substr($opdata,$i,2)));
        $opdata = $str;
    ...
    public function storeData($data)
    {
        ...
        */
        ...
        for ($data_ptr = 0; $data_ptr<$data_len; $data_ptr+=MAX_BYTES) {
            ...
            $outputs[$change_address['result']] = $change_amount;
    ...
    public function createTransaction($inputs, $outputs, $metadata, $metadata_pos) {
        ...
        $txn_unpacked =  $this->unpackTx(pack('H*', $raw_txn['result']));
    ...
    public function signAndSend($raw_txn) {
        ...
        if (!$signed_txn['result']['complete'])
            return array('error' => 'Could not sign the transaction');

        $send_txid = $this->rpcClient->sendRawTransaction($signed_txn['result']['hex']);
        //print_r($send_txid);
        if (strlen($send_txid['result']) != 64)
            return array('error' => 'Could not send the transaction txid:'.$send_txid.' raw : '.$send_txid);

        return array('txid' => $send_txid['result']);
    }
    ...
    public function selectInputs($total_amount) {
        ...
        //print_r($unspent_inputs[0]);
        if (!is_array($unspent_inputs))
            return array('error' => 'Could not retrieve list of unspent inputs');

        $unspent_inputs_new= array();
        foreach ($unspent_inputs as $index => $unspent_input) {
            for($i=0;$i<count($unspent_input);$i++){
                if (isset($unspent_input[$i]['amount'])){
                    $unspent_inputs_new[$index] = $unspent_input[$i];
                    $unspent_inputs_new[$index]['priority'] = $unspent_input[$i]['amount'] * $unspent_input[$i]['confirmations'];
                }
            }
        }
        // see: https://en.bitcoin.it/wiki/Transaction_fees
        Sorter::sortBy($unspent_inputs_new, 'priority');
        $unspent_inputs = array_reverse($unspent_inputs_new); // now in descending order of priority
    ...
    public function getAddressPay($dataprice){
        if (!$this->rpcClient->check())
            ...
            return $this->rpcClient->getNewAddress();
        }
